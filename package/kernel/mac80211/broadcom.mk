...
ifneq ($(CONFIG_B43_FW_DSL_3580L),)
  ...
  # which none of the broadcom-wl tarballs already supported here contain.
  # fwcutter knows this blob via patches-100-add-dsl3580l-blob.patch in
  # tools/b43-tools (vendored, pending upstream merge in mbuesch/b43-tools).
  PKG_B43_FWV4_NAME:=DSL-3580_EU
  PKG_B43_FWV4_VERSION:=1.00_10232013_GPL
  # Top-level dir name inside the GPL tarball assumed to match
  # the tarball base name. If it differs, override PKG_B43_FWV4_OBJECT
  # accordingly after running 'tar tzf' on the downloaded archive.
  ...
  if PACKAGE_kmod-b43
	...
	config B43_FW_DSL_3580L
		...
		help
		  ...
		  N-PHY (d11 corerev 22 + radio 2057 rev 8). The OEM 6.30-family
		  driver blob from the DSL-3580L GPL release is the only known
		  source for ucode22_mimo / n0initvals22 / n0bsinitvals22, which
		  none of the broadcom-wl tarballs above contain.

		  Select this if the target board uses the BCM6362 SoC integrated
		  WLAN (e.g. D-Link DSL-3580L). Other Broadcom chips supported by
		  b43 are NOT covered by this blob and need one of the broadcom-wl
		  firmware versions above instead.

		  Requires the vendored b43-fwcutter patch
		  100-add-dsl3580l-blob.patch in tools/b43-tools/patches/.
